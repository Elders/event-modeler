...
- `src/ports/` — the abstractions the use-cases speak to: `canvas` (the `Canvas` interface + the `CanvasElement` snapshot model and `ElementPatch`), `store`, `notifier`, `viewport`, `runtime`. Ports import only domain types.
...
- `src/index.ts` / `src/app.tsx` — composition roots: build `createMiroServices()`, `configureServices()`, then wire runtime events / mount the panel. Swapping the adapter set here is all it takes to host the tool elsewhere.
