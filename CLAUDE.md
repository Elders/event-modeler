...
- `src/features/` — the **use-cases** (one module per feature: `stickies`, `automation`, `screens`, `slices`, `createBlock`, `connectors`, `patterns`, `swimlane`, `chapter`, `convert`, `completeness`, `registerDrop`, `generate` (build a whole model from a `ModelPlan`; the build is interruptible and resumable — progress is checkpointed to the `em-gen` board-app-data key after each slice/link/spec, so Stop or a reload can resume from where it stopped), `generateCheckpoint`, `plannerSettings`, plus `helpers`/`assets`), and `src/features/specs/` (`model` = domain re-export + store-backed reads, `create`, `copies`, `reflow`, `selection`, `housekeeping`). Features read ports via `services()`; they never reference `miro` or `adapters/`.
...
- `src/panel/` — React components (`Panel` + `PanelTabs` (a **Build** tab for the manual palette and a **Generate** tab for AI) + one component per section, `Swatch`, `Dot`, `useBusyGuard`), each with co-located plain CSS. Panel calls features, never ports/adapters directly.
