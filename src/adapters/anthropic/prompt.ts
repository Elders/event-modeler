                  ...
                  items: {
                    ...
                    properties: {
                      ...
                      optional: {
                        type: 'boolean',
                        description:
                          'True when the field may be absent. Shown as a ? after the type and not required from blocks pointing in. Omit for required fields.',
                      },
                    },
                    required: ['name', 'type'],
                  },
...
- A field that may be absent (a nullable column, an optional form input) gets "optional": true — it renders as "name : type?" on the board. Leave "optional" out for required fields.
- Information flows along the arrows: a field a block holds must come from a block pointing into it, and a field keeps the SAME name and type everywhere it travels (an "orderId : uuid" on a command stays "orderId : uuid" on the event it produces) — the completeness check only matches on name, but keeping the type consistent too is what makes the model make sense. The blocks pointing into a block supply its fields TOGETHER, so a block fed by several others doesn't need any single one of them to carry everything — a read model hydrated by three events may take a different slice of its fields from each. The board runs an information-completeness check — if a block declares a required field that NONE of the blocks pointing into it provides as required (matched by name), every arrow into that block turns red.
- Optionality is checked on BOTH sides. A block's optional field never has to be supplied by anyone. But an optional field also SUPPLIES nothing: a field that may be absent cannot satisfy a downstream field that must be present. So never mark a field optional upstream and then require it downstream — if "email : string?" on an event feeds a read model needing "email : string", the arrow turns red. Either carry it as required from where it originates, or keep it optional all the way down.
- So introduce each field where it originates and carry it forward unchanged, and make sure each block's required fields are covered as required by its incoming blocks between them, so the generated model comes out complete (no red arrows).
