...
- Information flows along the arrows: a field a block holds must come from a block pointing into it, and a field keeps the SAME name and type everywhere it travels (an "orderId : uuid" on a command stays "orderId : uuid" on the event it produces). The blocks pointing into a block supply its fields TOGETHER, so a block fed by several others doesn't need any single one of them to carry everything — a read model hydrated by three events may take a different slice of its fields from each. The board runs an information-completeness check — if a block declares a required field that NONE of the blocks pointing into it provides as required (matched by name and type), every arrow into that block turns red.
